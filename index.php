...
add_action( 'widgets_init', create_function( '', 'register_widget( __NAMESPACE__ . "\UpcomingRacesWidget" );' ) );
 ...
 */
function display_next_race() : void {
	wp_reset_postdata();
 ...
 */
function display_race_details( $title = true ) : void  {
	global $post;
...
function bikereg_form_shortcode( $attributes, $event_id = null ) {
	if ( is_numeric( $event_id ) ) :
		$html = '<div class="' . echo esc_attr( $class ) . '" style="max -width:800px; margin:auto;" id="regWrapper"> ' .
 ...
 */
function create_races_post_type() : void {
	register_post_type(
		'races',
		array(
		'labels' => array(
 ...
 */
function race_metabox_setup() : void {
 ...
 *  our "results"
 */
function add_categories_for_attachments() : void {
